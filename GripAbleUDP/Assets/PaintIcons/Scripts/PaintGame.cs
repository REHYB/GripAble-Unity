...
public class PaintGame : MonoBehaviour {
    ...
    void Update() {
         if (GripablePlugin.Player.IsInitialized() == false && inputTypeSelected == true && inputType == 1) {
        ...
        // Get Gripable Data
        else if (GripablePlugin.Player.IsInitialized() && inputTypeSelected == true && inputType == 1) {
            if (initialize == false) {
        ...
        if (Time.time > timePrev + timeUpdate && inputTypeSelected == true && inputType == 2) {
        ...
        // Select bone height
        if (protocol == 3 && applyUserID == true && Time.time > (previousRespawn + respawnGap)) {
            ...
            //calibration
                ...
                if (trials % 2 == 0) {
                    ...
                    if (trials > 5) { 
                        if (success == true) {
                            success = false;
                            challengeTap = challengeTap - 0.55f * 1 / trials;
            ...
            //game
                ...
                if (trials % 2 == 0) {
                    appleHeight = appleHeightVector[UnityEngine.Random.Range(1, 7)];
                    reward = UnityEngine.Random.Range(1, 7); // 1 to 6 bones
                    Instantiate(apple, new Vector3(14.5f, appleHeight, 0), Quaternion.identity);
                    Instantiate(apple, new Vector3(14.5f, appleHeightVector[0], 0), Quaternion.identity);
                }
                else {
