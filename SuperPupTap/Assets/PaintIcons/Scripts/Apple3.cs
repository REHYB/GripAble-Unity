...
public class Apple3 : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D col) {
        if (col.gameObject.name == "climber" && boneContact == false) {
            boneContact = true;
            PaintGame.tagDestroy = trialTag;
    ...
    void Explode() {
        ...
        if (boneContact == true && boneCounted == false) {
            boneCounted = true;
            if (GetComponent<SpriteRenderer>().sprite == bone1) { PaintGame.bonesCaught = 1; }
            else if (GetComponent<SpriteRenderer>().sprite == bone2) { PaintGame.bonesCaught = 2; }
            else if (GetComponent<SpriteRenderer>().sprite == bone3) { PaintGame.bonesCaught = 3; }
            else if (GetComponent<SpriteRenderer>().sprite == bone4) { PaintGame.bonesCaught = 4; }
            else if (GetComponent<SpriteRenderer>().sprite == bone5) { PaintGame.bonesCaught = 5; }
            else if (GetComponent<SpriteRenderer>().sprite == bone6) { PaintGame.bonesCaught = 6; }
            exp.Play();
        }
